...
function loadSplitACProducts() {
    fetch('data/products.json')
        ...
        .then(products => {
            // Filter only cassette AC products
            allProducts = products.filter(product => product.type === 'cassette');
        ...
        .catch(error => {
            console.error('Error loading Cassete AC products:', error);
            showErrorState();
        });
}
...
function updateResultsCount() {
    ...
    if (countElement) {
        countElement.textContent = `${filteredProducts.length} Cassete AC product${filteredProducts.length !== 1 ? 's' : ''} found`;
    }
}
