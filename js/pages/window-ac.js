...
function loadSplitACProducts() {
    fetch('data/products.json')
        ...
        .then(products => {
            // Filter only window AC products
            allProducts = products.filter(product => product.type === 'window');
...
function updateResultsCount() {
    ...
    if (countElement) {
        countElement.textContent = `${filteredProducts.length} Window AC product${filteredProducts.length !== 1 ? 's' : ''} found`;
    }
}
